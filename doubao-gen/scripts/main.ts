...
Options:
  ...
  --help, -h              Show this help menu
    ...
    if (imageUrl) {
        ...
        // Use download URL from browser event if captured, otherwise use the image URL
        const finalUrl = client.downloadUrl || imageUrl;
        if (client.downloadUrl) {
            console.log(`\n🎯 使用浏览器下载事件捕获的URL`);
        }
        
        // Download the image (single attempt — template URL always works)
        const savedPath = await DoubaoClient.downloadImage(finalUrl, outputPath);
        
        if (savedPath) {
            const sizeKB = (fs.statSync(savedPath).size / 1024).toFixed(1);
            console.log(`💾 图片已保存至: ${savedPath} (${sizeKB} KB)`);
        } else {
